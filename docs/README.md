# Documentation Hub

Utah Flux Studio is the primary product. Documentation is split by **language** and **audience**.

## English

- Index: [docs/en/README.md](en/README.md)
- Architecture: [docs/en/architecture.md](en/architecture.md)
- UtahClaw canvas: [docs/en/utah-claw-studio.md](en/utah-claw-studio.md)
- Omniscient deck: [docs/en/omniscient-studio.md](en/omniscient-studio.md)
- Omega defense stack: [docs/en/omega-defense-stack.md](en/omega-defense-stack.md)
- Field graph compiler: [docs/en/field-graph-compiler.md](en/field-graph-compiler.md)
- Tutorials: [docs/en/tutorials/](en/tutorials/)

## 中文

- 索引：[docs/zh/README.md](zh/README.md)
- 架构摘要：[docs/zh/architecture.md](zh/architecture.md)
- UtahClaw 工作室：[docs/zh/utah-claw-studio.md](zh/utah-claw-studio.md)
- 全知甲板：[docs/zh/omniscient-studio.md](zh/omniscient-studio.md)
- Omega 防御栈：[docs/zh/omega-defense-stack.md](zh/omega-defense-stack.md)
- 场图编译器：[docs/zh/field-graph-compiler.md](zh/field-graph-compiler.md)
- 教程：[docs/zh/tutorials/](zh/tutorials/)

## Published docs site

MkDocs builds this content for GitHub Pages (see `mkdocs.yml`).
